			...
			for (unsigned int y = 0; y < height; y++)
			{
				if (state1.puzzle[x + y*width] != 0)
				{
					...
					switch (weight)
					{
						...
						case kInverse: man_dist += absDist*(1.0 / movingTile); break;
						case kSquared: man_dist += absDist*(movingTile)*(movingTile); break;
						case kLinear: man_dist += absDist*(movingTile); break;
...
double MNPuzzle<width, height>::GCost(const MNPuzzleState<width, height> &a, const MNPuzzleState<width, height> &b) const
{
	...
	switch (weight)
	{
		...
		case kInverse: return 1.0 / a.puzzle[b.blank];
		case kSquared: return a.puzzle[b.blank]*a.puzzle[b.blank];
		case kLinear: return a.puzzle[b.blank];
...
double MNPuzzle<width, height>::GCost(const MNPuzzleState<width, height> &s, const slideDir &d) const
{
	switch (weight)
	{
		...
		case kInverse:
		{
			switch (d)
			{
			case kLeft: return  1.0 / s.puzzle[s.blank - 1];
			case kUp: return 1.0 /  s.puzzle[s.blank - width];
			case kDown: return  1.0 / s.puzzle[s.blank + width];
			case kRight: return 1.0 /  s.puzzle[s.blank + 1];
			}
		}
		case kSquared:
		{
