#ifndef HEAVYPANCAKE_H
#define HEAVYPANCAKE_H
...
// Pancake puzzle state and action are defined in pancakepuzzle.h
...
template <int N>
class HeavyPancakePuzzle : public PermutationPuzzle::PermutationPuzzleEnvironment<PancakePuzzleState<N>, PancakePuzzleAction> {
public:
	HeavyPancakePuzzle(int _cost_weight =1, int gap = 0);
	HeavyPancakePuzzle(const std::vector<unsigned> op_order); // used to set action order
	...
	double GCost(const PancakePuzzleState<N> &, const PancakePuzzleState<N> &) const;
	double GCost(const PancakePuzzleState<N> &, const PancakePuzzleAction &) const;
...
template <int N>
double HeavyPancakePuzzle<N>::GCost(const PancakePuzzleState<N>  & s1, const PancakePuzzleState<N> & s2) const
{
	if (cost_weight == 1)
		return 1.0;
	...
	if (cost_weight == 2)
	{
		...
		return std::max(v_1, v_i);
	}
}

template <int N>
double HeavyPancakePuzzle<N>::GCost(const PancakePuzzleState<N> &s, const PancakePuzzleAction &a) const
{
	if(cost_weight==1)
		return 1.0; 
...
template <int N>
HeavyPancakePuzzle<N>::HeavyPancakePuzzle(int _cost_weight,int gap)
:cost_weight(_cost_weight),gap(gap)
{
	assert(N >= 2);
...
template <int N>
HeavyPancakePuzzle<N>::HeavyPancakePuzzle(const std::vector<PancakePuzzleAction> op_order)
:gap(0)
{
...
template <int N>
HeavyPancakePuzzle<N>::~HeavyPancakePuzzle()
{
	ClearGoal();
}

template <int N>
const std::string HeavyPancakePuzzle<N>::GetName(){
...
template <int N>
void HeavyPancakePuzzle<N>::GetSuccessors(const PancakePuzzleState<N> &parent,
...
template <int N>
void HeavyPancakePuzzle<N>::GetActions(const PancakePuzzleState<N> &, std::vector<PancakePuzzleAction> &actions) const
{
	actions.resize(0);
...
template <int N>
PancakePuzzleAction HeavyPancakePuzzle<N>::GetAction(const PancakePuzzleState<N> &parent, const PancakePuzzleState<N> &child) const
{
...
template <int N>
void HeavyPancakePuzzle<N>::ApplyAction(PancakePuzzleState<N> &s, PancakePuzzleAction action) const
{
	assert(s.size() == N);
...
template <int N>
bool HeavyPancakePuzzle<N>::InvertAction(PancakePuzzleAction &a) const
{
...
template <int N>
double HeavyPancakePuzzle<N>::HCost(const PancakePuzzleState<N> &state) const
{
	if (!goal_stored)
	{
...
template <int N>
double HeavyPancakePuzzle<N>::HCost(const PancakePuzzleState<N> &state, const PancakePuzzleState<N> &goal_state) const
{
//	if (state.size() != N)
//	{
...
template <int N>
double HeavyPancakePuzzle<N>::DefaultH(const PancakePuzzleState<N> &state) const
{
	return DefaultH(state, goal_locations);
}

template <int N>
double HeavyPancakePuzzle<N>::DefaultH(const PancakePuzzleState<N> &state, const std::vector<int> &goal_locs) const
{
//	if (state.size() != N)
//	{
//		fprintf(stderr, "ERROR: HCost called with state with wrong size.\n");
...
//	}
	...
	else if (cost_weight == 2)
	{
		...
		for (; i < N - 1; i++)
		{
			...
			int diff = goal_locs[state.puzzle[i]] - goal_locs[state.puzzle[i + 1]];
			if (diff > 1 || diff < -1)
				h_count = h_count + std::min(goal_locs[state.puzzle[i]], goal_locs[state.puzzle[i+1]]);
		}
		if ((unsigned)goal_locs[state.puzzle[i]] != N - 1)
			h_count = h_count + goal_locs[state.puzzle[i]];
	}
...
template <int N>
bool HeavyPancakePuzzle<N>::GoalTest(const PancakePuzzleState<N> &state, const PancakePuzzleState<N> &theGoal) const
{
	return (state == theGoal);
}

template <int N>
bool HeavyPancakePuzzle<N>::GoalTest(const PancakePuzzleState<N> &s) const
{
	if (!goal_stored)
	{
...
template <int N>
uint64_t HeavyPancakePuzzle<N>::GetActionHash(PancakePuzzleAction act) const
{
	return (uint64_t) act;
}

template <int N>
void HeavyPancakePuzzle<N>::StoreGoal(PancakePuzzleState<N> &g)
{
...
template <int N>
void HeavyPancakePuzzle<N>::Change_Op_Order(const std::vector<PancakePuzzleAction> op_order)
{
	operators.clear();
...
template <int N>
int HeavyPancakePuzzle<N>::read_in_pancake_puzzles(const char *filename, bool puzz_num_start, unsigned max_puzzles, std::vector<PancakePuzzleState<N>> &puzzles)
{
...
template <int N>
bool HeavyPancakePuzzle<N>::Path_Check(PancakePuzzleState<N> start, PancakePuzzleState<N> theGoal, std::vector<PancakePuzzleAction> &actions)
{
...
template <int N>
std::vector<unsigned> HeavyPancakePuzzle<N>::Get_Puzzle_Order(int64_t order_num, unsigned num_pancakes)
{
...
template <int N>
void HeavyPancakePuzzle<N>::OpenGLDraw(const PancakePuzzleState<N> &pps) const
{
